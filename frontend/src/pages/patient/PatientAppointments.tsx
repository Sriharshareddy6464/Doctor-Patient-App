    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="mb-2">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h2 className="text-2xl text-black font-semibold leading-tight tracking-tight">My Appointments</h2>
        </div>
        <p className="text-sm text-[#555555] mt-1">Track your consultations and join video calls</p>
      </div>
      ...
      {appointments.length === 0 ? (
        <div className="bg-white border border-[#e1e1e1] rounded-sm p-12 text-center">
          ...
          <p className="text-[#777777] text-sm mt-1 mb-6">Browse doctors to book your first consultation.</p>
          <Link
            to="/patient-dashboard/doctors"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-black border border-[#e1e1e1] bg-white hover:bg-[#efefef] px-4 py-2 rounded-sm transition-colors"
          >
            Find a Doctor
          </Link>
        </div>
                  ...
                  <tr key={appt.id} className="hover:bg-[#fcfcfc] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-7 h-7 rounded-sm border border-[#e1e1e1] bg-[#f5f5f5] flex-shrink-0 flex items-center justify-center text-black">
                          <User size={14} />
                        </div>
                        <div>
                          <div className="font-medium text-black">Dr. {appt.doctor?.name}</div>
                          {appt.doctor?.doctorProfile?.specializations?.length ? (
                            <div className="text-[#777777] text-[11px] font-mono">
                              {appt.doctor.doctorProfile.specializations.slice(0, 2).join(', ')}
                            </div>
                          ) : null}
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      {appt.status === 'CONFIRMED' && (
                        ...
                        >
                          <Video size={12} />
                          {appt.callStatus === 'IN_PROGRESS' ? 'Rejoin' : 'Join Call'}
                        </button>
                      )}
                    </td>
                  </tr>
